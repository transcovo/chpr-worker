...
const {
  applyConfiguration,
  applyOptions
} = require('./schema/createWorkers.schema');
...
function createWorkers(handlers, config, options = {}) {
  ...
  /**
   * Wait for an event for a given amout of time.
   *
   ...
   * @param {Object} params the call params
   * @param {String} params.amqpUrl the url to the server (ex: 'amqp://localhost')
   * @param {String} params.workerName the name of the worker (for logs)
   * @param {Number} params.heartbeat the period of the connection heartbeat, in seconds
  ...
  function* _connectAmqp({ amqpUrl, workerName, heartbeat }, socketOptions = {}) {
    try {
      ...
      subscribeToConnectionEvents(connection, workerName);
      return connection;
    } catch (err) {
      logger.error(
