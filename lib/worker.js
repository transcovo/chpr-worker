...
function createWorker(handler, config, options = {}) {
  ...
  const configWithoutFuncs = _.omit(configuration, ['amqpUrl', 'handler', 'validator']);
  ...
  function* _consumeMessage(message) {
    ...
    logger.info({ message }, '[worker#listen] received message');
    const channel = _getChannel();

    const content = _parseMessage(message);
    if (!content) return channel.ack(message);

    const isValid = _validateMessage(content);
    if (!isValid) return channel.ack(message);

    const handleSuccess = yield _handleMessage(content, message.fields);
  ...
  function _parseMessage(message) {
    try {
      ...
      logger.info({ message, contentString }, '[worker#listen] message content');
   ...
   * @returns {boolean} true if validator did not throw an error, false else
   * @private
   */
  function _validateMessage(message) {
    const { workerName, validator } = configuration;
    try {
      validator(message);
      return true;
    } catch (err) {
   ...
   * @private
   */
  function* _handleMessage(content, fields) {
    const { workerName, taskTimeout, handler: messageHandler } = configuration;
    try {
      yield _promisifyWithTimeout(messageHandler(content, fields), workerName, taskTimeout);
      return true;
    } catch (err) {
