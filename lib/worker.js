 ...
 * @param {function|Array} handlers function or array of handlers to handle each message
 ...
 */
function createWorker(handlers, config, options = {}) {
  ...
  function _getMessageConsumer(handle, validate) {
    return function* _consumeMessage(message) {
      ...
      const isValid = _validateMessage(validate, content);
      if (!isValid) return channel.ack(message);

      const handleSuccess = yield _handleMessage(handle, content, message.fields);
